...
from backend.api.service_dependencies import CommunityEngagementServiceDep
...
async def like_post(post_id: str, current_user: TokenPayload = Depends(get_current_user),
                    service: CommunityEngagementServiceDep = None) -> Response:
    """Like a community post. Idempotent - liking twice has no effect."""
    try:
        await service.like_post(current_user.sub, post_id)
...
async def unlike_post(post_id: str, current_user: TokenPayload = Depends(get_current_user),
                      service: CommunityEngagementServiceDep = None) -> Response:
    """Unlike a community post. No error if not previously liked."""
    await service.unlike_post(current_user.sub, post_id)
    return Response(status_code=status.HTTP_204_NO_CONTENT)
...
async def list_comments(
    ...
    service: CommunityEngagementServiceDep = None,
) -> PaginatedCommentsResponse:
    """List comments on a post with pagination."""
    viewer_id = current_user.sub if current_user else None
    comments, total = await service.list_comments(post_id, page, limit, viewer_id)
...
async def create_comment(
    ...
    try:
        return await service.create_comment(current_user.sub, post_id, data)
    except (CommunityPostNotFoundError, CommunityUserBannedError) as e:
        _handle_exception(e)
...
async def edit_comment(
    ...
    try:
        return await service.edit_comment(current_user.sub, comment_id, data)
...
async def delete_comment(comment_id: str, current_user: TokenPayload = Depends(get_current_user),
                         service: CommunityEngagementServiceDep = None) -> Response:
    """Delete a comment. Owner or post owner can delete."""
    try:
        await service.delete_comment(current_user.sub, comment_id)
        return Response(status_code=status.HTTP_204_NO_CONTENT)
...
async def follow_user(user_id: str, current_user: TokenPayload = Depends(get_current_user),
                      service: CommunityEngagementServiceDep = None) -> Response:
    """Follow a user. Idempotent - following twice has no effect."""
    try:
        await service.follow_user(current_user.sub, user_id)
        return Response(status_code=status.HTTP_204_NO_CONTENT)
...
async def unfollow_user(user_id: str, current_user: TokenPayload = Depends(get_current_user),
                        service: CommunityEngagementServiceDep = None) -> Response:
    """Unfollow a user. No error if not previously following."""
    await service.unfollow_user(current_user.sub, user_id)
    return Response(status_code=status.HTTP_204_NO_CONTENT)
...
async def get_followers(
    ...
    users, total = await service.get_followers(user_id, page, limit)
...
async def get_following(
    ...
    users, total = await service.get_following(user_id, page, limit)
    return PaginatedUsersResponse(items=users, total=total, page=page, limit=limit, has_more=(page * limit) < total)
...
async def get_creator_profile(
    ...
    viewer_id = current_user.sub if current_user else None
    return await service.get_creator_profile(user_id, viewer_id)
