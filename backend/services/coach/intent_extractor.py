    ...
    def is_valid(self) -> bool:
        ...
        for pattern in placeholder_patterns:
            if re.search(pattern, desc_lower, re.IGNORECASE):
                logger.warning(f"Intent contains placeholder text: {self.description[:100]}")
                return False
...
class IntentExtractor:
    ...
    EXTRACTION_PATTERNS: List[Tuple[str, str]] = [
        ...
        # Must have content after Ready, not just "Ready to create?"
        (r"(?:✨\s*)?Ready[!:]\s+([A-Z][^?]+?)(?:\[INTENT_READY\]|$)", "ready_marker"),
        # "Perfect! A high-energy thumbnail"
        (r"(?:✨\s*)?Perfect[!:]\s+([A-Z][^?]+?)(?:\[INTENT_READY\]|$)", "perfect_marker"),
        # "Here's what I've got: a cool thumbnail"
        (r"Here's what I've got:\s*(.+?)(?:\.(?:\s|$)|Ready|\[INTENT_READY\])", "heres_what"),
        # "So we're going for a neon-style emote"
        (r"So we're going for\s*(.+?)(?:\.(?:\s|$)|Ready|\[INTENT_READY\])", "going_for"),
        # "we'll create a dynamic banner"
        (r"we'll create\s*(.+?)(?:\.(?:\s|$)|Ready|\[INTENT_READY\])", "well_create"),
        # "Updated: a dynamic banner with neon effects"
        (r"Updated:\s*(.+?)(?:\.(?:\s|$)|Ready|\[INTENT_READY\])", "updated"),
        # "Your vision: a sleek gaming thumbnail"
        (r"Your vision:\s*(.+?)(?:\.(?:\s|$)|Ready|\[INTENT_READY\])", "your_vision"),
        # "Creating: a bold emote with fire effects"
        (r"Creating:\s*(.+?)(?:\.(?:\s|$)|Ready|\[INTENT_READY\])", "creating"),
    ]
    ...
    def extract_from_conversation(
        ...
        building on the previous intent.
        ...
        for pattern, method_name in self._compiled_patterns:
            ...
            if match:
                ...
                if extracted and len(extracted) >= self.MIN_DESCRIPTION_LENGTH:
                    logger.debug(f"Extracted intent using '{method_name}': {extracted[:50]}...")
    ...
    def _extract_additions(self, message: str) -> str:
        ...
        fillers = [
            "yes", "yeah", "sure", "ok", "okay", "sounds good",
            "perfect", "great", "awesome", "nice", "cool",
            "let's do it", "go for it", "that works",
        ]
