...
class SessionNotFoundError(Exception):
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "message": f"Session {self.session_id} not found or expired",
...
class SessionExpiredError(Exception):
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "message": f"Session {self.session_id} has expired. Please start a new session.",
            "session_id": self.session_id,
        }
...
class SessionLimitExceededError(Exception):
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "message": f"{self.limit_type} limit exceeded",
...
class SessionManager:
    ...
    async def check_limits(self, session: CoachSession) -> Dict[str, Any]:
        ...
        """
        return {
            "turns_exceeded": session.turns_used >= self.MAX_TURNS,
            "tokens_in_exceeded": session.tokens_in_total >= self.MAX_TOKENS_IN,
            "tokens_out_exceeded": session.tokens_out_total >= self.MAX_TOKENS_OUT,
            "turns_remaining": max(0, self.MAX_TURNS - session.turns_used),
            "can_continue": (
                session.turns_used < self.MAX_TURNS and
                session.tokens_in_total < self.MAX_TOKENS_IN and
                session.tokens_out_total < self.MAX_TOKENS_OUT and
    ...
    async def refresh_ttl(self, session_id: str) -> bool:
        ...
        """
        key = self._key(session_id)
        result = await self.redis.expire(key, self.SESSION_TTL)
        return result
