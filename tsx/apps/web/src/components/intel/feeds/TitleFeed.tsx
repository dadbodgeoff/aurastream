...
import { Type, Flame, TrendingUp, Copy, Check, Zap, Clock, RefreshCw } from 'lucide-react';
...
function CopyButton({ text }: { text: string }) {
  ...
  return (
    <button 
      onClick={handleCopy} 
      className="p-1.5 hover:bg-white/10 rounded transition-colors flex-shrink-0 opacity-0 group-hover:opacity-100"
      title="Copy title"
    >
      {copied ? <Check className="w-3.5 h-3.5 text-green-400" /> : <Copy className="w-3.5 h-3.5 text-text-tertiary" />}
    </button>
  );
}
  ...
  return (
    <div className="space-y-2">
      ...
      {trendingHooks.length > 0 && (
        <div className="p-3 bg-orange-500/10 border border-orange-500/20 rounded-xl sticky top-0 z-10 backdrop-blur-sm">
