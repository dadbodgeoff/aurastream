 ...
 * Promo Page Redirect
 * 
 * Redirects to /community?tab=promo
 * Shows one-time toast notification about the move.
 * 
 * Part of UX Consolidation 2025 - Task 3.7
 */
export default function PromoRedirectPage() {
  const router = useRouter();
  const { showToast, dismissToast, message } = useRedirectToast('promo');

  useEffect(() => {
    // Use replace for soft redirect (307-like behavior, doesn't add to history)
    router.replace('/community?tab=promo');
  }, [router]);
