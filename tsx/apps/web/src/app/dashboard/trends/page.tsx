...
/**
 * Trends Redirect Page
 * 
 * Redirects to /dashboard/intel?tab=trends
 * Shows one-time toast notification about the move.
 * 
 * Part of UX Consolidation 2025 - Task 4.6
 */
export default function TrendsRedirectPage() {
  const router = useRouter();
  const { showToast, dismissToast, message } = useRedirectToast('trends');

  useEffect(() => {
    // Use replace for soft redirect (307-like behavior, doesn't add to history)
    router.replace('/dashboard/intel?tab=trends');
  }, [router]);
